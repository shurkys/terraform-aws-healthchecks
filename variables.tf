...
variable "healthchecks" {
  description = "Map of values passed to Healthchecks container definition. See the [ECS container definition module](https://github.com/terraform-aws-modules/terraform-aws-ecs/tree/master/modules/container-definition) for full list of arguments supported"
  type        = any
  default     = {}
}

variable "healthchecks_gid" {
  description = "GID of the healthchecks user"
  type        = number
  default     = 1000
}

variable "healthchecks_uid" {
  description = "UID of the healthchecks user"
  type        = number
  default     = 100
}
...
variable "alb_target_group_arn" {
  description = "ARN of an existing ALB target group that will be used to route traffic to the Healthchecks service. Required if `create_alb` is `false`"
  type        = string
  default     = ""
}
...
variable "alb_internal" {
  description = "Internet-facing (false) or Internal (true). An internet-facing load balancer routes requests from clients to targets over the internet. An internal load balancer routes requests to targets using private IP addresses."
  type        = bool
  default     = false
}
...
variable "alb_security_group_ingress_rules" {
  description = "Custom ingress rules for alb security group"
  type = map(object({
    from_port = number
    to_port   = number
    protocol  = string
